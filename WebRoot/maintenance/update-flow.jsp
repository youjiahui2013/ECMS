...
<script language="javascript"> 
	...
	function radCheck()
	{
		var tmpValue = document.getElementById('id_hidden1').value 
		
		var radArr =document.getElementsByName('flow.ftype');
		
		for(var i = 0 ;i < radArr.length;i++)
		{
			if(radArr[i].value == tmpValue)
			{
				radArr[i].checked="checked";
				break;
			}
		}
	}
...
<script type="text/javascript">
	function check(){
		...
		var fids=document.getElementById("flowids").value;
		var m=0;
		if(rightval.length!=0){
	 		for(var i=0;i<rightval.length;i++){
	 			m=m+1;
				addtr(rightval.options[i].value,fids,m)
			}
			return true;
		}else{
			alert("没有选到审批人");
...
function addtr(mv,mc,mk){
		...
		for (j=0; j <activeTable.rows(iRow-1).cells.length; j++){
			...
			switch(j){
				case 0:
				activeTable.rows[iRow].cells(j).innerHTML =mv+"<input name='flowSeqList["+list+"].seq.sid' style='visibility:hidden;border-left:none;border-right:none;border-top:none;border-bottom:none;' readonly='readonly' value='"+mv+"' >";
				break; 
				case 1:
	           activeTable.rows[iRow].cells(j).innerHTML ="<input name='flowSeqList["+list+"].flow.fid' style='visibility:hidden;border-left:none;border-right:none;border-top:none;border-bottom:none;' readonly='readonly' value='"+mc+"' >";
	            break;
				case 2:
	           activeTable.rows[iRow].cells(j).innerHTML ="<input name='flowSeqList["+list+"].flowsequence' style='visibility:hidden;border-left:none;border-right:none;border-top:none;border-bottom:none;' readonly='readonly' value='"+mk+"' >";
...
-->
	...
	<body style="background-color: Silver" onload="radCheck();">
	    <h1 align="center" style="background-color:aqua;font-size:34px;font-weight:bold;color:Black">
				审批流程管理
		</h1><br />

				修改审批流程
		<form action="<%=request.getContextPath()%>/flow/update-flow.action" name="form1" onsubmit="return check();">
		
		<%int n=Integer.parseInt(request.getParameter("fid")); %>
 
		  		<input type="hidden" name="flow.fid" id ="flowids" value="<%=n %>" readonly="readonly">
			
			<s:iterator var="FLOWID" value="flowIDList">
	  		
			审批流程名：
			<input type="text" name="flow.fname" value="${FLOWID.fname }"/><br><br>
			审批流程类型：
			<input type="radio" name="flow.ftype" value="${FLOWID.ftype }" />预算&nbsp;
			<input type="radio" name="flow.ftype" value="${FLOWID.ftype }"/>报销<br><br>

			是否先由财务审批（有时预算流程如此）：
			<input type="radio" name="inp1" id="id_inp" value="ok" onclick="setValue()"/>
			是&nbsp;
			<input type="radio" name="inp1" id="id_inp" value="no" onclick="clearValue()"/>
			否
			<br><br>
			</s:iterator>
						...
						<td>
							<div id="divchar">
								<select multiple="multiple" id="leftList"
									...
									<s:iterator var="SEQLIST" value="seqList">
									<option value="${SEQLIST.sid}">${SEQLIST.name }</option>
								</s:iterator>
								</select>
							</div>
						</td>
		...
		<input type="hidden" value="${FLOWID.ftype }" id="id_hidden1"/>	
	</body>
</html>
